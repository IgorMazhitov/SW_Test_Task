...
import { PASSWORD_SALT_LENGTH } from './constants/constants';
...
export class UsersService extends BaseService {
  ...
  async createUser(dto: CreateUserDto): Promise<IUserResponse> {
    return await this.executeWithErrorHandling(async () => {
      ...
      const hashPassword = await crypt.hash(dto.password, PASSWORD_SALT_LENGTH);
  ...
  async changeUser(dto: ChangeUserDto): Promise<IUserResponse> {
    return await this.executeWithErrorHandling(async () => {
      ...
      if (dto.password !== user.password) {
        userToUpdate.password = await crypt.hash(
          dto.password,
